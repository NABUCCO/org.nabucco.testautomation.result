...
public class DeleteTestConfigurationResultHandlerImpl extends
        AbstractDeleteDatatypeHandler<TestConfigurationResultMaintenanceMultiPageEditView> implements
        DeleteTestConfigurationResultHandler {
    ...
    @Override
    protected boolean preClose(TestConfigurationResultMaintenanceMultiPageEditView view) throws ClientException {

        TestConfigurationResult testConfigResult = view.getModel().getTestConfigurationResult();

        testConfigResult.setDatatypeState(DatatypeState.DELETED);

        BusinessModel businessModel = Activator.getDefault().getModel()
                .getBusinessModel(TestConfigurationResultEditViewBusinessModel.ID);

        if (businessModel instanceof TestConfigurationResultEditViewBusinessModel) {
            ((TestConfigurationResultEditViewBusinessModel) businessModel).delete(testConfigResult);
        }
        
        // Remove HistoryEntry
        HistoryService.getInstance().removeHistoryEntry(testConfigResult.getIdentificationKey());
