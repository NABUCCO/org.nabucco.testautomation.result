 ...
 * 
 */
public class TestConfigurationResultSearchBusinessModel implements
	 ...
	 */
	@Override
	public TestConfigurationResultListViewBrowserElement search(
		...
		if (searchViewModel instanceof TestConfigurationResultSearchViewModel) {
			...
			if(searchResult != null){
				result = new TestConfigurationResultListViewBrowserElement(searchResult.toArray(new TestConfigurationResult[0]));
			}
		}
		return result;
	}
	...
	private List<TestConfigurationResult> search(final TestConfigurationResultSearchMsg rq) {
		...
		try {
			SearchTestConfigurationResultDelegate searchDelegate = ResultComponentServiceDelegateFactory
					.getInstance().getSearchTestConfigurationResult();
